...
df['blood_group'] = np.array(blood_group,
dtype=int).reshape(1000,1)
m = len(df)
...
if k == 1000:
k = 0
self.theta[j] = self.theta[j] - (self.alpha/m)*np.sum(
((self.hypothesis(x) - y))*(x[k,:][j]) )
...
print("Theta 2 = ",t[2])
print("Theta 3 = ",t[3])
print("Enter the blood group of the employee (A or B or AB or
O) - ", end = "")
blood_group = input()
if blood_group == 'A':
blood_group = 0
...
age = int(input())
print("Enter the eating habit of the employee on a scale of 0 -
10 - ", end = "")
healthy_eating = int(input())
print("Enter the life style of the employee on a scale of 0 -
10 - ", end = "")
active_lifestyle = int(input())

salary = t[0] + t[1]*age + t[2]*healthy_eating +
t[3]*active_lifestyle + t[4]*blood_group
